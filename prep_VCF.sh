...
for dir in *_SNP; 
   ...
   do         
      ...
      sbatch -t 0:10:00 --wrap="ml bcftools;bcftools view -R /scratch4/wisaacs1/hg19_Exome_splice.bed  $file -O z -o /scratch4/wisaacs1/FILT-SNP/$filename";
   done
done
...
for dir in *_INDEL; 
   ...
   do         
      filename=$(basename $file);         
      sbatch -t 0:10:00 --wrap="ml bcftools;bcftools view -R /scratch4/wisaacs1/hg19_Exome_splice.bed  $file -O z -o /scratch4/wisaacs1/FILT-INDEL/$filename";     done; done

#Now concatenate the SNP and INDEL files from the same individual
for file in *.indel.vcf.gz
do    
   sampleID=$(basename "$file" | cut -d '.' -f 1);    
   sbatch -t 20:00 -J $sampleID --wrap="ml bcftools; bcftools concat -a $sampleID.GATK.indel.vcf.gz /scratch4/wisaacs1/FILT-SNP/$sampleID.GATK.snp.vcf.gz -O z -o /scratch4/wisaacs1/Concat/$sampleID.vcf.gz" 
done 
